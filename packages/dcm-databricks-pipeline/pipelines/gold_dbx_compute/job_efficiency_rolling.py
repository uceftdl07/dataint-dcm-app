"""Agregation gold `gold_dbx_compute_job_efficiency_rolling` (efficacite job, fenetres).

Rollup 1/7/30/90 jours de `gold_dbx_compute_job_efficiency_daily` : pour chaque
job, utilisation CPU/memoire et diagnostic de dimensionnement recalcules sur les
derniers `window_days` jours "as of" le dernier jour disponible. Miroir exact de
`cluster_efficiency_rolling` au grain stable `job_id`.

ATTENTION -- ce builder ne lit QUE `gold_dbx_compute_job_efficiency_daily`, qui
derive elle-meme de `gold_dbx_compute_cluster_efficiency_daily`. Cette derniere
NE DOIT PAS etre filtree sur `cluster_type = 'ALL_PURPOSE'` (seule
`cluster_efficiency_rolling` porte ce filtre, cf. SC-001) : un tel filtre
viderait toute la chaine d'efficacite job.

Regles de derivation :
  - Percentiles : recalcules a partir des histogrammes quotidiens
    (`cpu_util_hist`/`mem_util_hist`) sommes bucket par bucket sur la fenetre,
    ...
    restent internes (CTE) et ne sont pas materialises en sortie.
  ...
  - Sommes additives : `uptime_hours`, `active_hours`, `autoscale_oscillation`,
    `estimated_savings_usd`, `cluster_count`.
  - Maxima : `worker_count_max`.
  - Diagnostics (`utilization_status`, `rightsizing_reco`) : memes seuils que la
    table quotidienne. PAS de `is_zombie` (cf. `job_efficiency_daily`).
  - Attributs descriptifs (`job_name`, `driver_node_type`, `worker_node_type`,
    `recommended_node_type`) et configuration de taille : repris au dernier jour
    connu.
...
def build_job_efficiency_rolling(
    spark: SparkSession,
    *,
    job_efficiency_daily_table: str,
    rolling_windows: tuple[int, ...] = ROLLING_WINDOWS,
) -> DataFrame:
    """Construit `gold_dbx_compute_job_efficiency_rolling`.

    Args:
        spark: session Spark utilisee pour executer la requete generee.
        job_efficiency_daily_table: nom qualifie (`catalog.schema.table`) de
            `gold_dbx_compute_job_efficiency_daily` (unique source).
        rolling_windows: fenetres glissantes (jours) a materialiser, une ligne
            par job ET par fenetre (defaut `ROLLING_WINDOWS`).

    Returns:
        Le DataFrame `gold_dbx_compute_job_efficiency_rolling` resultant.

    Grain : `(cloud_provider, workspace_id, job_id, window_days)`. Snapshot
    "as of" le dernier jour disponible (`as_of_date = MAX(period_start)`).

    Aucun filtre de `cluster_type` ici : la source est DEJA restreinte aux
    clusters `JOB` par `job_efficiency_daily`.

    Un job sans aucun jour dans la fenetre COURANTE n'est pas materialise (la
    CTE `agg` pilote la sortie, cf. `WHERE m.uptime_hours > 0`) : ses colonnes de
    fenetre precedente ne sont donc jamais exposees seules.

    `cluster_count` est SOMME sur la fenetre (et non `MAX`) : les clusters JOB
    sont ephemeres, un `cluster_id` par execution, donc les ensembles de
    clusters de deux jours differents sont disjoints par construction -- la
    somme donne bien le nombre de clusters distincts de la fenetre.
    """
    ...
    WITH daily AS (
        ...
        FROM {job_efficiency_daily_table}
    ),
    anchor AS (
    ...
    latest_attrs AS (
        SELECT
            ...
            driver_node_type,
            worker_node_type,
            -- Taille VOULUE au dernier jour connu (cf. job_efficiency_daily) :
    ...
    agg AS (
        SELECT
            ...
            SUM(d.cluster_count) AS cluster_count,
        ...
        GROUP BY d.cloud_provider, d.workspace_id, d.job_id, w.window_days, a.as_of_date
    ),
    -- Fenetre PRECEDENTE de meme longueur, dans une CTE dediee plutot qu'en
    -- elargissant `agg` a 2 * window_days (cf. cluster_efficiency_rolling).
    ...
    prev_agg AS (
        ...
        GROUP BY d.cloud_provider, d.workspace_id, d.job_id, w.window_days
    ),
    with_metrics AS (
        SELECT
            g.*,
    ...
    SELECT
        ...
        -- Diagnostics recalcules sur la fenetre (memes seuils que job_efficiency_daily).
        CASE
    ...
    -- window_days FAIT PARTIE de la cle : prev_agg porte une ligne par job ET
