"""Agregation gold `gold_dbx_compute_pipeline_efficiency_rolling` (DLT, fenetres).

Rollup 1/7/30/90 jours de `gold_dbx_compute_pipeline_efficiency_daily` : pour
chaque pipeline Lakeflow/DLT, utilisation CPU/memoire et diagnostic de
dimensionnement recalcules sur les derniers `window_days` jours "as of" le
dernier jour disponible. Miroir exact de `job_efficiency_rolling` au grain
stable `dlt_pipeline_id`.

ATTENTION -- ce builder ne lit QUE
`gold_dbx_compute_pipeline_efficiency_daily`, qui derive elle-meme de
`gold_dbx_compute_cluster_efficiency_daily`. Cette derniere NE DOIT PAS etre
filtree sur `cluster_type = 'ALL_PURPOSE'` (seule `cluster_efficiency_rolling`
porte ce filtre, cf. SC-001) : un tel filtre viderait toute la chaine
d'efficacite DLT.

Regles de derivation : identiques a `job_efficiency_rolling` (percentiles
recalcules depuis les histogrammes sommes, moyennes ponderees par
`uptime_hours`, sommes additives, pas de `is_zombie`, fenetre precedente `NULL`
et jamais `0`).
"""
...
def build_pipeline_efficiency_rolling(
    spark: SparkSession,
    *,
    pipeline_efficiency_daily_table: str,
    rolling_windows: tuple[int, ...] = ROLLING_WINDOWS,
) -> DataFrame:
    """Construit `gold_dbx_compute_pipeline_efficiency_rolling`.

    Args:
        spark: session Spark utilisee pour executer la requete generee.
        pipeline_efficiency_daily_table: nom qualifie (`catalog.schema.table`)
            de `gold_dbx_compute_pipeline_efficiency_daily` (unique source).
        rolling_windows: fenetres glissantes (jours) a materialiser, une ligne
            par pipeline ET par fenetre (defaut `ROLLING_WINDOWS`).

    Returns:
        Le DataFrame `gold_dbx_compute_pipeline_efficiency_rolling` resultant.

    Grain : `(cloud_provider, workspace_id, dlt_pipeline_id, window_days)`.
    Snapshot "as of" le dernier jour disponible
    (`as_of_date = MAX(period_start)`).

    Aucun filtre de `cluster_type` ici : la source est DEJA restreinte aux
    clusters `PIPELINE` par `pipeline_efficiency_daily`.

    Un pipeline sans aucun jour dans la fenetre COURANTE n'est pas materialise
    ...
    `cluster_count` est SOMME sur la fenetre (et non `MAX`) : les clusters
    `PIPELINE` sont ephemeres, un `cluster_id` par mise a jour, donc les
    ensembles de clusters de deux jours differents sont disjoints par
    construction.
    """
    ...
    WITH daily AS (
        ...
        FROM {pipeline_efficiency_daily_table}
    ),
    anchor AS (
    ...
    latest_attrs AS (
        SELECT
            ...
            driver_node_type,
            worker_node_type,
            -- Taille VOULUE au dernier jour connu (cf.
            -- pipeline_efficiency_daily) : a ne pas confondre avec
            -- worker_count_avg/worker_count_max, agreges sur la fenetre a partir
            -- des tailles observees par minute.
            autoscale_enabled,
            autoscale_min_workers,
            autoscale_max_workers,
            configured_worker_count,
    ...
    agg AS (
        ...
        GROUP BY d.cloud_provider, d.workspace_id, d.dlt_pipeline_id, w.window_days, a.as_of_date
    ),
    ...
    prev_agg AS (
        ...
        GROUP BY d.cloud_provider, d.workspace_id, d.dlt_pipeline_id, w.window_days
    ),
    with_metrics AS (
        SELECT
            g.*,
    ...
    SELECT
        ...
        -- Diagnostics recalcules sur la fenetre (memes seuils que
    ...
    -- window_days FAIT PARTIE de la cle : prev_agg porte une ligne par pipeline
    -- ET par fenetre, joindre sans lui multiplierait chaque ligne de sortie par 4.
    LEFT JOIN prev_agg p
      ON p.cloud_provider = m.cloud_provider
     AND p.workspace_id = m.workspace_id
     AND p.dlt_pipeline_id = m.dlt_pipeline_id
