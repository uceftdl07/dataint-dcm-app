"""Point d'entree de la tache wheel Gold Workspace (`[project.scripts]`).

Symetrique a `pipelines.gold_dbx_compute.entrypoint` (meme mecanique : resolution
de la `SparkSession` et des parametres, deux modes cluster/local ; aucune partie
Azure/secrets, uniquement des tables Unity Catalog deja qualifiees). Ce plugin ne
calcule ni n'ecrit de table : il (re)cree la vue `dim_dbx_workspace` via
`spark.sql(build_dim_dbx_workspace_view_sql(catalog, schema))`.
"""
...
def main(spark: Any, params: dict[str, str]) -> None:  # noqa: ANN401
    """Orchestration : (re)cree la vue `dim_dbx_workspace` (SQL pur, idempotent).

    Fonction testable sans cluster reel (spark peut etre un fake enregistrant les
    appels `sql()`). Qualifie la vue et ses tables source via `catalog`/`schema`
    ...
    (meme garde-fou que `pipelines.gold_dbx_compute.entrypoint.main`).
    """
    ...
    if not catalog or not schema:
        raise ValueError(
            ...
            "dim_dbx_workspace et ses tables source (sinon DDL dans le catalog/schema "
            "par defaut de la session au lieu de la cible Unity Catalog)."
        )
    view_sql = build_dim_dbx_workspace_view_sql(catalog, schema)
    logging.getLogger("pipelines").info(
        "Creation/mise a jour de la vue %s.%s.dim_dbx_workspace", catalog, schema
    )
    spark.sql(view_sql)
