...
import { Activity, Cpu, DollarSign, RefreshCw, ShieldAlert, Workflow } from 'lucide-react';
import {
...
import { ComputePipelineDrawer } from '../components/domain/compute/compute-pipeline-drawer';
...
import {
  useComputePipelineCostTrend,
  useComputePipelineDetail,
  useComputePipelineUptimeTrend,
  useComputePipelinesCostData,
  useComputePipelinesEfficiencyData,
  useComputePipelinesOverviewData,
} from '../hooks/useComputePipelinesQueries';
...
import type {
  ...
  ComputePipelineCostItem,
  ComputePipelineEfficiencyItem,
  ComputePipelinesOverviewItem,
} from '../types/api';
...
/** The pipeline name, with the id below and as fallback when the name is empty. */
function PipelineCell({ name, id }: { name: string | null | undefined; id: string }) {
  return (
...
/** Cost rank within the window, with a badge on the top-cost pipeline. */
function RankCell({ item }: { item: ComputePipelineCostItem }): ReactNode {
  return (
 ...
 *
 * An absent value is "not measured", never `0`: the first window of a pipeline has
 * no predecessor, and the backend nulls the value and its delta together.
 */
...
/**
 * Sizing verdict as a badge, rendered like the all-purpose page: an unmeasured
 * pipeline — a serverless one has no cluster to observe — reads `—` in an outline
 * badge rather than being dropped from the column. No zombie variant at this grain:
 * a PIPELINE cluster dies with its update (024 R8).
 */
...
export default function ComputePipelines() {
  ...
  const [selectedPipeline, setSelectedPipeline] = useState<{ id: string; title: string } | null>(
    null
  );
  ...
  const overview = useComputePipelinesOverviewData({
  ...
  const cost = useComputePipelinesCostData({
    window_days: windowDays,
    search: costSearch,
    sort: costSort,
    page: costPage,
  ...
  const efficiency = useComputePipelinesEfficiencyData({
    window_days: windowDays,
    search: effSearch,
    sort: effSort,
    page: effPage,
  ...
  const pipelineDetail = useComputePipelineDetail(selectedPipeline?.id ?? null, windowDays);
  const pipelineCostTrend = useComputePipelineCostTrend({
    pipelineId: selectedPipeline?.id ?? null,
    granularity: drawerGranularity,
    ...drawerPeriod,
  });
  const pipelineUptimeTrend = useComputePipelineUptimeTrend({
    pipelineId: selectedPipeline?.id ?? null,
  ...
  const openPipeline = useCallback(
    (row: { dlt_pipeline_id: string; pipeline_name: string | null }) => {
      setSelectedPipeline({
        id: row.dlt_pipeline_id,
        title: row.pipeline_name || row.dlt_pipeline_id,
      });
    },
    []
  );
  const closePipeline = useCallback(() => setSelectedPipeline(null), []);
  ...
  /**
   * Averages over the visible page, computed on measured rows only: a serverless
   * pipeline has no `node_timeline` sample and must not pull the average toward 0.
   */
  ...
  const workspaceColumn = useMemo(
    (): ComputeDataTableColumn<ComputePipelineCostItem> => ({
      id: 'workspace',
      width: COLUMN_WIDTH.identifier,
      header: 'Workspace',
      description: 'Databricks workspace the pipeline runs in.',
  ...
  const columns = useMemo(
    (): ComputeDataTableColumn<ComputePipelinesOverviewItem>[] => [
      workspaceColumn,
      {
        id: 'pipeline',
        width: COLUMN_WIDTH.name,
        header: 'Pipeline',
        description: 'DLT / Lakeflow pipeline — its ephemeral clusters are billed to it.',
        sortable: true,
        cell: (row) => <PipelineCell name={row.pipeline_name} id={row.dlt_pipeline_id} />,
      },
      {
        id: 'cost',
        width: COLUMN_WIDTH.metric,
        header: 'Cost',
        description: 'Total pipeline cost (update + maintenance) over the window.',
        align: 'right',
        sortable: true,
        cell: (row) => (
      ...
      },
      {
        ...
        header: 'Lifetime',
        description: 'Cumulated uptime of the pipeline clusters over the window.',
        align: 'right',
        cell: (row) => (
      ...
      },
      {
        ...
        header: 'Utilization',
        description:
          'Sizing verdict of the pipeline clusters. A pipeline billed without a node timeline — serverless — is unmeasured and reads “—”.',
    ...
    (): ComputeDataTableColumn<ComputePipelineCostItem>[] => [
      {
        ...
        header: 'Rank',
        description: 'Cost rank within the window — 1 is the most expensive pipeline.',
      ...
      },
      {
        ...
        width: COLUMN_WIDTH.number,
        header: 'DBU',
        description: 'Databricks Units consumed by the pipeline over the window.',
        align: 'right',
        cell: (row) => (
  ...
  const effColumns = useMemo(
    (): ComputeDataTableColumn<ComputePipelineEfficiencyItem>[] => [
      {
        id: 'workspace',
        width: COLUMN_WIDTH.identifier,
        header: 'Workspace',
      ...
      },
      {
        ...
        header: 'Pipeline',
        description:
          'DLT / Lakeflow pipeline — the utilization of its clusters is aggregated to it.',
        sortable: true,
        cell: (row) => <PipelineCell name={row.pipeline_name} id={row.dlt_pipeline_id} />,
      },
      {
        ...
        header: 'Clusters',
        description:
          'Pipeline clusters measured over the window. The Cost tab counts none: pipeline cost is billed directly to the pipeline.',
        align: 'right',
        cell: (row) => (
      ...
      },
      {
        ...
        header: 'Driver node',
        description: 'Driver instance type of the pipeline clusters over the window.',
        cell: (row) => (
      ...
      },
      {
        ...
        width: COLUMN_WIDTH.timestamp,
        header: 'Worker node',
        description: 'Worker instance type of the pipeline clusters over the window.',
        cell: (row) => (
      ...
      },
      {
        ...
        header: 'Autoscaling',
        description: 'Whether the pipeline clusters autoscale.',
        cell: (row) =>
      ...
      },
      {
        ...
        header: 'Uptime',
        description:
          'Uptime cumulated over the pipeline updates of the window — not a cluster lifetime.',
        align: 'right',
        sortable: true,
        cell: (row) => (
          <span className="font-semibold tabular-nums">{formatHours(row.uptime_hours)}</span>
      ...
      },
      {
        ...
        width: COLUMN_WIDTH.metric,
        header: 'Idle',
        description: 'Share of the measured uptime with no work on the pipeline clusters.',
        align: 'right',
        cell: (row) => (
      ...
      },
      {
        ...
        width: COLUMN_WIDTH.number,
        header: 'CPU avg',
        description: 'Average CPU utilization of the pipeline clusters over the window.',
        align: 'right',
        cell: (row) => (
      ...
      },
      {
        ...
        width: COLUMN_WIDTH.number,
        header: 'Mem avg',
        description: 'Average memory utilization of the pipeline clusters over the window.',
        align: 'right',
        cell: (row) => (
      ...
      },
      {
        ...
        description: 'Instance type the rightsizing rule suggests for the pipeline clusters.',
        cell: (row) => (
  ...
  const sortKey = sort === 'name' ? 'pipeline' : 'cost';
  const handleSortChange = (key: string) => {
    setSort(key === 'pipeline' ? 'name' : 'cost_desc');
  };

  const costSortKey = costSort === 'name' ? 'pipeline' : 'cost';
  const handleCostSortChange = (key: string) => {
    setCostSort(key === 'pipeline' ? 'name' : 'cost_desc');
  };
  ...
  const handleEffSortChange = (key: string) => {
    if (key === 'pipeline') return setEffSort('name');
  ...
  return (
    <Content className="mx-auto max-w-[1600px] gap-0 p-4 pb-24 lg:px-6 lg:pb-28">
      <ContentHeader>
        <div>
          <ContentTitle>DLT pipeline clusters</ContentTitle>
          <p className="mt-2 text-sm text-muted-foreground">
            DLT / Lakeflow pipeline cost aggregated to the stable pipeline — {scope.label}.
          </p>
        </div>
        <Button
          variant="secondary"
      ...
      <ContentMain className="gap-5">
        <ComputeCostGrainTabs
          value={tab}
          onChange={setTab}
          ariaLabel="DLT pipeline clusters tabs"
        />
        ...
        {tab === 'overview' ? (
          <>
            <div className="grid grid-cols-1 gap-4 overflow-visible sm:grid-cols-2 xl:grid-cols-3">
              {overview.loading && !overview.data ? (
                ...
                <>
                  <ComputeKpiCard
                    title="Total cost"
                    description="Cost of the DLT pipelines running on a classic cluster, over the selected window. Serverless DLT is not in scope of this page."
                  ...
                  <ComputeKpiCard
                    ...
                    description="Pipelines billed over the window and served by a classic PIPELINE / PIPELINE_MAINTENANCE cluster."
                    value={formatNumber(overview.data?.kpis.active_pipelines ?? null)}
                    subtitle="Classic clusters · billed"
                    tone="success"
                    icon={Workflow}
                  />
                  <Link
                    to="/databricks/compute/recommendations?object_type=PIPELINE"
                    ...
                    aria-label="View pipeline recommendations"
                  >
                    <ComputeKpiCard
                      title="Recommendations & forecast"
                      description="Open the cost recommendations and forecast for pipelines."
                      value="View"
                      subtitle="Pipelines only · click to open"
                      tone="purple"
                      icon={ShieldAlert}
                    />
                  </Link>
            ...
            {items.length || searchActive ? (
              <ComputeDataTable
                tableId="compute-pipelines-overview"
                columns={columns}
                rows={items}
                rowKey={(row) => `${row.workspace_id}-${row.dlt_pipeline_id}`}
                loading={overview.loading && !overview.data}
                emptyTitle="No pipelines"
                emptyDescription="No DLT pipeline cost over this window and perimeter."
              ...
              <ComputeEmptyState
                title="No pipelines"
                description="No DLT pipeline cost over this window and perimeter."
              />
            )}
          </>
        ) : null}
        ...
        {tab === 'cost' ? (
          <>
            <div className="grid grid-cols-1 gap-4 overflow-visible sm:grid-cols-2 xl:grid-cols-3">
              ...
              <ComputeKpiCard
                title="Top costly"
                description="Most expensive pipeline over the window."
                value={topCostItem ? topCostItem.pipeline_name || topCostItem.dlt_pipeline_id : '—'}
            ...
            {costItems.length || costSearchActive ? (
              <ComputeDataTable
                tableId="compute-pipelines-cost"
                columns={costColumns}
                rows={costItems}
                rowKey={(row) => `${row.workspace_id}-${row.dlt_pipeline_id}`}
                ...
                toolbar={
                  <div className="relative w-full max-w-[280px] shrink-0">
                    <Input
                      ...
                      placeholder="Search pipeline name or id…"
                      className="h-8 rounded-full text-xs"
                      aria-label="Search pipelines cost"
                    />
                  </div>
                }
              />
        ...
        {tab === 'efficiency' ? (
          <>
            ...
            {effItems.length || effSearchActive ? (
              <ComputeDataTable
                tableId="compute-pipelines-efficiency"
                columns={effColumns}
                rows={effItems}
                rowKey={(row) => `${row.workspace_id}-${row.dlt_pipeline_id}`}
                loading={efficiency.loading && !efficiency.data}
                emptyTitle="No measured pipelines"
                emptyDescription="No DLT pipeline utilization over this window and perimeter."
                onRowClick={openPipeline}
                ...
                toolbar={
                  <div className="relative w-full max-w-[280px] shrink-0">
                    <Input
                      ...
                      placeholder="Search pipeline name or id…"
                      className="h-8 rounded-full text-xs"
                      aria-label="Search pipelines efficiency"
                    />
                  </div>
                }
              />
              ...
              <ComputeEmptyState
                title="No measured pipelines"
                description="No DLT pipeline utilization over this window and perimeter. A serverless pipeline has no node timeline and is only listed on the Cost tab."
              />
            )}
          </>
        ) : null}
      ...
      <ComputePipelineDrawer
        open={Boolean(selectedPipeline)}
        onClose={closePipeline}
        title={selectedPipeline?.title ?? ''}
        detail={pipelineDetail.data}
        detailLoading={pipelineDetail.loading}
        detailError={pipelineDetail.error}
        trend={pipelineCostTrend.data}
        trendLoading={pipelineCostTrend.loading}
        trendError={pipelineCostTrend.error}
        granularity={drawerGranularity}
        onGranularityChange={setDrawerGranularity}
        uptimeTrend={pipelineUptimeTrend.data}
        uptimeTrendLoading={pipelineUptimeTrend.loading}
        uptimeTrendError={pipelineUptimeTrend.error}
        uptimeGranularity={drawerUptimeGranularity}
        onUptimeGranularityChange={setDrawerUptimeGranularity}
        workspaceLabel={
          pipelineDetail.data?.workspace_id
            ? resolveWithId(pipelineDetail.data.workspace_id).label
            : undefined
        }
      />
    </Content>
